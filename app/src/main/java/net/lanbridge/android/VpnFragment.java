...
public final class VpnFragment extends ToolFragment {
    ...
    private TextView text(String value,int size,String color) { TextView v=new TextView(context());v.setText(value);v.setTextSize(size);v.setTextColor(Color.parseColor(color));v.setLineSpacing(dp(3),1);return v; }
    ...
    private void connect() {
        ...
        if(phase.equals("connecting")||phase.equals("connected")) { startService(new Intent(context(),BridgeVpnService.class).setAction(BridgeVpnService.DISCONNECT));return; }
        try {
    ...
    private void prepareVpn() { Intent approval=VpnService.prepare(context());if(approval!=null) startActivityForResult(approval,10);else begin(); }
    ...
    private void begin() {
        ...
        BridgeVpnService.pending=credentials;credentials=null;startForegroundService(new Intent(context(),BridgeVpnService.class).setAction(BridgeVpnService.CONNECT));
    }

...
public final class RelayFragment extends ToolFragment {
    ...
    private void more(View anchor){PopupMenu menu=new PopupMenu(context(),anchor,Gravity.END);String[] labels={"排序","选择文件","新建文件夹","按时间清理","上传日志","WebDAV 与备份"};for(int i=0;i<labels.length;i++)menu.getMenu().add(0,i,i,labels[i]);menu.setOnMenuItemClickListener(item->{switch(item.getItemId()){
    ...
    private void settings(){page="settings";header("WebDAV 与备份");content.addView(text("文件中转站自动使用统一连接和根目录，无需单独设置路径。",14));button(content,"打开统一设置",()->navigate("webdav"));}
    ...
    private boolean transfer(String label,Operation operation,List<Uri> grants,int flags){
        try{
            ...
            final WebDavSettings.Connection connection=WebDavSettings.connection(context());final String url=connection.url,folder=connection.root+"/file-relay",user=connection.user,password=connection.password;
            if(password.isEmpty())throw new IOException("请先保存 WebDAV 连接");
            RelayService.submit(context(),()->{try(RelayDav client=new RelayDav(url,folder,user,password)){RelayService.bind(client);RelayService.message=label;connection.ensure();operation.run(client);RelayService.message=label+"：已完成";if(label.startsWith("上传"))log(label+"：已完成");}catch(Exception e){if(label.startsWith("上传"))log(label+"："+e.getMessage());throw e;}},grants,flags);
            return true;

...
public final class LedgerSync {
    ...
    public String remotePath(){return context.getSharedPreferences("ledger",0).getString("remote_path","WinToolbox");}
    public void path(String value)throws Exception{RelayDav.path(value,false);context.getSharedPreferences("ledger",0).edit().putString("remote_path",value).apply();sync();}
    ...
    private void perform(){
        ...
        try{
            ...
            String identity=credentials.origin()+"\n"+credentials.username()+"\n"+remotePath()+"\n"+credentials.revision();if(!identity.equals(verifiedIdentity)){verifiedOps.clear();verifiedIdentity=identity;}
            LedgerStore local=store();status="正在合并同步…";
            try(RelayDav dav=new RelayDav(credentials.origin(),remotePath(),credentials.username(),credentials.password(credentials.origin(),credentials.username()))){

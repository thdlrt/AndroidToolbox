...
public final class RelayActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state){super.onCreate(state);try{WebDavSettings.migrate(this);}catch(Exception e){notice(e.getMessage());}store=WebDavSettings.store(this);preferences=getSharedPreferences("relay",0);connectionIdentity=WebDavSettings.identity(this);if(state!=null){ArrayList<Uri> savedIncoming=state.getParcelableArrayList("incoming");if(savedIncoming!=null)incoming.addAll(savedIncoming);filter=state.getString("filter","");sort=state.getInt("sort",0);path=state.getString("path","");page=state.getString("page","files");selecting=state.getBoolean("selecting");ArrayList<String> savedSelected=state.getStringArrayList("selected");if(savedSelected!=null)selected.addAll(savedSelected);String saved=state.getString("download");if(saved!=null)pendingDownload=new RelayDav.Entry(saved,false,state.getLong("size"),state.getLong("modified"),state.getString("etag",""));}
        ...
        scroll=new ScrollView(this);scroll.setFillViewport(true);content=column();content.setPadding(dp(20),dp(8),dp(20),dp(20));scroll.addView(content);shell.addView(scroll,new LinearLayout.LayoutParams(-1,0,1));footer=column();footer.setPadding(dp(20),0,dp(20),dp(12));shell.addView(footer);setContentView(new ToolUi.Shell(this,shell,"relay"));if(state==null)receive(getIntent());String restoredPage=page;showFiles();if(!store.hasPassword()||restoredPage.equals("settings"))settings();else if(restoredPage.equals("downloads"))downloads();else if(restoredPage.equals("logs"))logs();else load();sendIncoming();}
    @Override protected void onNewIntent(Intent i){super.onNewIntent(i);setIntent(i);receive(i);path="";filter="";selecting=false;entries=new ArrayList<>();selected.clear();showFiles();if(!store.hasPassword())settings();else{load();sendIncoming();}}
    ...
    private void header(String title){content.removeAllViews();chrome.removeAllViews();footer.removeAllViews();status=null;selectionBar=null;scroll.scrollTo(0,0);LinearLayout row=new LinearLayout(this);row.setGravity(Gravity.CENTER_VERTICAL);row.addView(ToolUi.iconButton(this,"back",page.equals("files")?"返回工具箱":"返回中转文件",()->{if(page.equals("files"))finish();else{showFiles();load();}}));TextView heading=ToolUi.text(this,title,23,ToolUi.INK);heading.setTypeface(null,android.graphics.Typeface.BOLD);row.addView(heading,new LinearLayout.LayoutParams(0,-2,1));chrome.addView(row);if(page.equals("files"))row.addView(ToolUi.iconButton(this,"more","更多操作",()->more(row)));}
    private RelayDav dav()throws Exception {String password=store.password(store.origin(),store.username());if(password.isEmpty())throw new IOException("请在设置中配置 WebDAV 连接");return new RelayDav(store.origin(),WebDavSettings.path(this),store.username(),password);}
    private void showFiles(){page="files";header("文件中转站");
        LinearLayout tabs=new LinearLayout(this);tabs.setGravity(Gravity.CENTER_VERTICAL);Button remote=ToolUi.button(this,"中转文件",false,()->{});tabs.addView(remote,new LinearLayout.LayoutParams(0,dp(44),1));Button local=ToolUi.button(this,"已下载",false,this::downloads);ToolUi.ripple(local,Color.TRANSPARENT,12);local.setTextColor(ToolUi.MUTED);tabs.addView(local,new LinearLayout.LayoutParams(0,dp(44),1));chrome.addView(tabs);
        LinearLayout searchRow=new LinearLayout(this);searchRow.setGravity(Gravity.CENTER_VERTICAL);searchRow.setPadding(dp(12),0,dp(4),0);searchRow.setBackground(ToolUi.box(this,0xffeaf0f7,14));searchRow.addView(ToolUi.icon(this,"search",ToolUi.MUTED),new LinearLayout.LayoutParams(dp(20),dp(20)));
        ...
        LinearLayout location=new LinearLayout(this);location.setGravity(Gravity.CENTER_VERTICAL);if(!path.isEmpty())location.addView(ToolUi.iconButton(this,"back","上一级",this::up));breadcrumb=ToolUi.text(this,path.isEmpty()?"中转根目录":path,13,ToolUi.MUTED);breadcrumb.setMaxLines(2);breadcrumb.setEllipsize(android.text.TextUtils.TruncateAt.MIDDLE);location.addView(breadcrumb,new LinearLayout.LayoutParams(0,-2,1));location.addView(ToolUi.iconButton(this,"refresh","刷新",this::load));chrome.addView(location);
        if(!incoming.isEmpty())button(content,"上传待传的 "+incoming.size()+" 个文件",this::sendIncoming);
        files=column();files.setBackground(ToolUi.box(this,Color.WHITE,18));content.addView(files);
        LinearLayout primary=new LinearLayout(this);Button upload=ToolUi.button(this,"上传文件",true,this::pickUpload);primary.addView(upload,new LinearLayout.LayoutParams(-1,dp(50)));footer.addView(primary);
        selectionBar=new LinearLayout(this);selectionBar.setGravity(Gravity.CENTER_VERTICAL);selectionBar.addView(ToolUi.iconButton(this,"close","取消选择",()->{selecting=false;selected.clear();render();}));selectionCount=ToolUi.text(this,"",14,ToolUi.INK);selectionBar.addView(selectionCount,new LinearLayout.LayoutParams(0,-2,1));selectionBar.addView(ToolUi.iconButton(this,"check","全选当前目录文件",()->{for(RelayDav.Entry e:visibleEntries())if(!e.directory)selected.add(e.path);render();}));selectionBar.addView(ToolUi.iconButton(this,"trash","删除所选",()->{List<RelayDav.Entry> chosen=new ArrayList<>();for(RelayDav.Entry e:entries)if(selected.contains(e.path))chosen.add(e);delete(chosen);}));footer.addView(selectionBar);
        selectionBar.setTag(primary);
        transferBar=new LinearLayout(this);transferBar.setGravity(Gravity.CENTER_VERTICAL);status=ToolUi.text(this,RelayService.status(),12,ToolUi.MUTED);status.setMaxLines(2);status.setPadding(0,dp(12),0,dp(8));status.setOnClickListener(v->logs());transferBar.addView(status,new LinearLayout.LayoutParams(0,-2,1));cancelTransfer=ToolUi.iconButton(this,"close","取消传输",RelayService::cancel);transferBar.addView(cancelTransfer);transferBar.setVisibility(View.GONE);footer.addView(transferBar);render();}
    ...
    private void more(View anchor){PopupMenu menu=new PopupMenu(this,anchor,Gravity.END);String[] labels={"排序","选择文件","新建文件夹","按时间清理","上传日志","中转目录"};for(int i=0;i<labels.length;i++)menu.getMenu().add(0,i,i,labels[i]);menu.setOnMenuItemClickListener(item->{switch(item.getItemId()){
        case 0:new AlertDialog.Builder(this).setTitle("排序").setSingleChoiceItems(new String[]{"最新在前","最旧在前","名称","大小"},sort,(d,w)->{sort=w;render();d.dismiss();}).show();break;
    ...
    private void render(){
        ...
        List<RelayDav.Entry> visible=visibleEntries();if(visible.isEmpty()){LinearLayout empty=column();empty.setGravity(Gravity.CENTER);empty.setPadding(dp(24),dp(48),dp(24),dp(48));empty.addView(ToolUi.icon(this,loadError.isEmpty()?"folder":"cloud",ToolUi.MUTED),new LinearLayout.LayoutParams(dp(44),dp(44)));TextView title=text(!loadError.isEmpty()?"暂时无法读取文件":listing?"正在加载…":!store.hasPassword()?"连接你的 WebDAV":filter.isEmpty()?"还没有中转文件":"没有匹配的文件",17);title.setGravity(Gravity.CENTER);empty.addView(title);TextView detail=text(!loadError.isEmpty()?loadError:listing?"":!store.hasPassword()?"在设置中配置一次即可使用":filter.isEmpty()?"上传文件，便可在其他设备接着使用":"试试其他文件名",13);detail.setTextColor(ToolUi.MUTED);detail.setGravity(Gravity.CENTER);empty.addView(detail);if(!loadError.isEmpty())button(empty,"重试",this::load);else if(!store.hasPassword())button(empty,"配置连接",()->startActivity(new Intent(this,WebDavActivity.class)));files.addView(empty);}
        for(RelayDav.Entry e:visible){LinearLayout row=new LinearLayout(this);row.setGravity(Gravity.CENTER_VERTICAL);row.setMinimumHeight(dp(80));row.setPadding(dp(12),dp(10),dp(8),dp(10));ToolUi.ripple(row,selected.contains(e.path)?0xffedf2ff:Color.WHITE,14);files.addView(row);
            if(selecting&&!e.directory){CheckBox check=new CheckBox(this);check.setContentDescription("选择 "+e.name);check.setChecked(selected.contains(e.path));row.addView(check,new LinearLayout.LayoutParams(dp(48),dp(48)));check.setOnCheckedChangeListener((b,on)->{if(on)selected.add(e.path);else selected.remove(e.path);render();});}
            else{String glyph=e.directory?"folder":RelayProvider.mime(e.name).startsWith("image/")?"image":"file";ImageView icon=ToolUi.icon(this,glyph,e.directory?0xffb47c26:ToolUi.BLUE);icon.setPadding(dp(10),dp(10),dp(10),dp(10));icon.setBackground(ToolUi.box(this,e.directory?0xfffff4df:0xffedf2ff,12));row.addView(icon,new LinearLayout.LayoutParams(dp(44),dp(48)));}
            LinearLayout details=column();details.setPadding(dp(12),0,dp(4),0);row.addView(details,new LinearLayout.LayoutParams(0,-2,1));TextView name=ToolUi.text(this,e.name,15,ToolUi.INK);name.setMaxLines(2);name.setEllipsize(android.text.TextUtils.TruncateAt.MIDDLE);details.addView(name);TextView meta=ToolUi.text(this,e.directory?"文件夹":size(e.size)+" · "+(e.modified==0?"时间未知":android.text.format.DateFormat.format("MM/dd HH:mm",e.modified)),12,ToolUi.MUTED);meta.setPadding(0,dp(6),0,0);details.addView(meta);
            ...
            if(!e.directory&&!selecting)row.addView(ToolUi.iconButton(this,"more","文件操作 "+e.name,()->actions(e)));
            View divider=new View(this);divider.setBackgroundColor(0xffedf1f6);LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(-1,dp(1));lp.leftMargin=dp(68);lp.rightMargin=dp(16);files.addView(divider,lp);
        }
    }
    private void load(){if(!store.hasPassword()||!alive())return;if(listing){reloadPending=true;return;}listing=true;loadError="";render();final String requested=path,identity=WebDavSettings.identity(this);final RelayDav client;try{client=dav();}catch(Exception e){listing=false;loadError=e.getMessage();render();return;}reads.execute(()->{try(RelayDav remote=client){List<RelayDav.Entry> value=remote.list(requested);ui(()->{if(path.equals(requested)&&identity.equals(WebDavSettings.identity(this))){entries=value;Set<String> present=new HashSet<>();for(RelayDav.Entry e:value)present.add(e.path);selected.retainAll(present);loadError="";}});}catch(Exception e){ui(()->{if(path.equals(requested)&&identity.equals(WebDavSettings.identity(this))){loadError=e.getMessage();if(!entries.isEmpty())notice(loadError);}});}finally{ui(()->{listing=false;render();if(reloadPending){reloadPending=false;load();}});}});}
    ...
    private void settings(){page="settings";header("中转目录");if(!incoming.isEmpty())content.addView(text("已接收 "+incoming.size()+" 个文件，配置后自动上传。",14));LinearLayout card=column();card.setPadding(dp(20),dp(16),dp(20),dp(20));card.setBackground(ToolUi.box(this,Color.WHITE,18));content.addView(card);card.addView(ToolUi.icon(this,"cloud",ToolUi.BLUE),new LinearLayout.LayoutParams(dp(32),dp(32)));card.addView(text(store.hasPassword()?"使用设置中的 WebDAV":"尚未配置 WebDAV",18));TextView hint=text("地址、账号和密码与全局设置共用。",14);hint.setTextColor(ToolUi.MUTED);card.addView(hint);button(card,"管理 WebDAV 连接",()->startActivity(new Intent(this,WebDavActivity.class)));
        EditText folder=ToolUi.field(this,content,"远端中转路径",WebDavSettings.path(this),false);folder.setId(303);folder.setHint("共享名/文件中转站");content.addView(text("相对于 WebDAV 地址，例如 shared/文件中转站。",13));button(content,"保存并打开",()->{try{String dir=RelayDav.path(folder.getText().toString().trim(),false);preferences.edit().putString("remote_path",dir).apply();plan.clear();path="";entries.clear();selected.clear();connectionIdentity=WebDavSettings.identity(this);if(!store.hasPassword()){startActivity(new Intent(this,WebDavActivity.class));return;}showFiles();transfer("测试连接",client->{client.ensure("");ui(this::sendIncoming);});}catch(Exception e){folder.setError(e.getMessage());}});}
    ...
    private boolean transfer(String label,Operation operation,List<Uri> grants,int flags){
        try{
            ...
            final String url=store.origin(),folder=WebDavSettings.path(this),user=store.username(),password=store.password(url,user);
    ...
    private void downloads(){page="downloads";header("已下载");TextView hint=text("本机副本，点击直接打开。删除缓存不会影响远端文件。",13);hint.setTextColor(ToolUi.MUTED);content.addView(hint);
        try{List<File> cached=RelayCache.list(getCacheDir());if(cached.isEmpty())content.addView(text("暂无下载文件",16));for(File file:cached){
            LinearLayout row=new LinearLayout(this);row.setGravity(Gravity.CENTER_VERTICAL);row.setPadding(dp(14),dp(14),dp(8),dp(14));ToolUi.ripple(row,Color.WHITE,16);LinearLayout.LayoutParams rp=new LinearLayout.LayoutParams(-1,-2);rp.topMargin=dp(10);content.addView(row,rp);
            row.addView(ToolUi.icon(this,RelayProvider.mime(file.getName()).startsWith("image/")?"image":"file",ToolUi.BLUE),new LinearLayout.LayoutParams(dp(28),dp(32)));
            LinearLayout details=column();details.setPadding(dp(14),0,dp(8),0);TextView name=ToolUi.text(this,file.getName(),15,ToolUi.INK);name.setMaxLines(2);name.setEllipsize(android.text.TextUtils.TruncateAt.MIDDLE);details.addView(name);TextView meta=ToolUi.text(this,size(file.length())+" · "+android.text.format.DateFormat.format("MM/dd HH:mm",file.lastModified()),12,ToolUi.MUTED);meta.setPadding(0,dp(6),0,0);details.addView(meta);row.addView(details,new LinearLayout.LayoutParams(0,-2,1));row.setContentDescription("打开缓存 "+file.getName());row.setFocusable(true);row.setOnClickListener(v->openCached(file,false));
            row.addView(ToolUi.iconButton(this,"more","缓存操作 "+file.getName(),()->new AlertDialog.Builder(this).setTitle(file.getName()).setItems(new String[]{"打开","分享给其他应用","删除本机缓存"},(d,w)->{if(w<2)openCached(file,w==1);else new AlertDialog.Builder(this).setTitle("删除本机缓存？").setMessage(file.getName()+"，远端文件保留。").setNegativeButton("取消",null).setPositiveButton("删除",(a,b)->{try{RelayCache.remove(getCacheDir(),file);downloads();}catch(Exception e){notice(e.getMessage());}}).show();}).show()));
    ...
    private void preview(){new AlertDialog.Builder(this).setTitle("清理超过多久未修改的文件？").setItems(new String[]{"1 天","1 周","30 天","90 天"},(d,index)->{
        final String identity=WebDavSettings.identity(this);
        transfer("清理预览",client->{List<RelayDav.Entry> value=client.cleanupPreview(new int[]{1,7,30,90}[index]);long created=System.currentTimeMillis();ui(()->{
            ...
            new AlertDialog.Builder(this).setTitle("清理预览").setMessage(names).setNegativeButton("取消",null).setPositiveButton("确认清理",(a,b)->{
                if(value.isEmpty())return;if(System.currentTimeMillis()-created>600000||!identity.equals(WebDavSettings.identity(this))){notice("预览已过期，请重新预览");return;}
    ...
    @Override protected void onResume(){super.onResume();String identity=WebDavSettings.identity(this);if(!identity.equals(connectionIdentity)){connectionIdentity=identity;path="";filter="";selecting=false;entries.clear();selected.clear();plan.clear();if(page.equals("files"))load();else if(page.equals("settings"))settings();}sendIncoming();handler.post(refresh);}
    @Override protected void onPause(){handler.removeCallbacks(refresh);super.onPause();}
    @Override protected void onDestroy(){reads.shutdown();super.onDestroy();}
    @Override public void onBackPressed(){if(!page.equals("files")){showFiles();load();}else if(selecting){selecting=false;selected.clear();render();}else if(!path.isEmpty())up();else super.onBackPressed();}
}
